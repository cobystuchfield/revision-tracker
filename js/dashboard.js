async function renderDashboard() {
  ...
  const [logs, settings] = await Promise.all([Store.getLogs(), Store.getSettings()]);
  ...
  weekLogs.forEach(l => { SUBJECTS.forEach(s => { weekBySubject[s.id] += (l.subjects[s.id] && l.subjects[s.id].totalTime) || 0; }); });
  ...
  el.innerHTML = `
    <div class="stat-grid">
      ...
      <div class="stat-card"><div class="stat-val">${days}</div><div class="stat-label">days logged</div></div>
      <div class="stat-card"><div class="stat-val">${avgPerDay}m</div><div class="stat-label">avg / day</div></div>
      <div class="stat-card"><div class="stat-val">${Math.round(weekMins/60*10)/10}h</div><div class="stat-label">this week (target ${settings.weeklyHours}h)</div></div>
    </div>
    ...
    <div class="card" style="margin-bottom:28px">
      ${SUBJECTS.map(s => {
        ...
        return `<div class="bar-row"><div class="bar-label">${s.label}</div><div class="bar-track"><div class="bar-fill ${fill}" style="width:${pct}%"></div></div><div class="bar-count">${pct}%</div></div>`;
      }).join('')}
    </div>
    ...
    <div class="exam-list">
      ${EXAMS.map(e => {
        ...
        const dClass = isPast?'done':d<=7?'urgent':d<=21?'soon':'ok';
        const dLabel = isPast?'done':d===0?'today':`${d}d`;
